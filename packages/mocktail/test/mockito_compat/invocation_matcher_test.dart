  ...
  group('$isInvocation', () {
    ...
    test('positional arguments (arg matcher)', () {
      ...
      shouldPass(
        isInvocation(call1).matches(
          Invocation.method(
            #say,
            [const ArgMatcher(anything, '', false)],
          ),
          <dynamic, dynamic>{},
        ),
        isTrue,
    ...
    test('type arguments', () {
      ...
      shouldFail(
        ...
        "Actual: <Instance of '${call3.runtimeType}'> "
        'Which: Does not match calculate<Type:<int>>()',
      );
    });

    test('optional arguments', () {
      stub.lie(facingDown: true);
      final call1 = Stub.lastInvocation;
      stub.lie(facingDown: true);
      final call2 = Stub.lastInvocation;
      stub.lie(facingDown: false);
      final call3 = Stub.lastInvocation;
      shouldPass(call1, isInvocation(call2));
      shouldFail(
        call1,
        isInvocation(call3),
        "Expected: lie('facingDown: false') "
        "Actual: <Instance of '${call3.runtimeType}'> "
        "Which: Does not match lie('facingDown: true')",
      );
    });
